...
class Workload(AbstractWorkload):
    ...
    def _run_model(self, input_data):
        """ Run the model on the given input data ; Return the accuracy"""

        f = input_data["data_file"]

        interpreter = Interpreter('./models/mobilenet_v1_1.0_224_quant.tflite',experimental_delegates=[tflite.load_delegate('libedgetpu.so.1')])
